...
const { ben_gsheetId, app: { url: tayaba_apiUrl } } = require(`../config/${env}.json`);
...
const lib = {
    async getKoboFormsData() {
        ...
        const { data: { data: currentVillages } } = await axios.get(
            `${tayaba_apiUrl}/api/v1/villages`
        );
        ...
        const { data: { data: allVillages } } = await axios.get(
            `${tayaba_apiUrl}/api/v1/villages`
        );
        ...
        for (let d of rows) {
            try {
                ...
                let { data: resData } = await axios.post(
                    `${tayaba_apiUrl}/api/v1/beneficiaries`,
                    data
                );
